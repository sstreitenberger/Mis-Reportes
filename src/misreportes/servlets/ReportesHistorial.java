 ...
 */
@WebServlet("/reportes_historial")
public class ReportesHistorial extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			request.setAttribute("menulist", new QuerysDAO().getListado(u,"historial") );
			request.setAttribute("abmRepoTipo", "historial");
			
			if (!u.getApp_nivel().equalsIgnoreCase(Info.PERMISOS_STORE_MANAGER)) {	
				request.setAttribute("tiendas", new TiendasDAO().getTiendas(u) );
				request.getRequestDispatcher("/reportes_general_historial.jsp").forward(request, response);
			} else {
				request.setAttribute("tienda", new TiendasDAO().getTiendaPorLocal(u));
				request.getRequestDispatcher("/reportes_locales_historial.jsp").forward(request, response);
			}
